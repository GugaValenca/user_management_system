...
describe("api refresh flow", () => {
  ...
  it("retries the original request with a new access token after a 401", async () => {
    ...
    tokenStore.setRefreshCsrfToken("csrf-value");
    ...
    rootMock.onPost(/\/auth\/refresh\/$/).reply((config) => {
      ...
      return [200, { access: "new-access-token", csrf_token: "new-csrf-value" }];
    });
    ...
    expect(tokenStore.getRefreshCsrfToken()).toBe("new-csrf-value");
  });
  ...
  it("sends the in-memory CSRF token as a header on logout", async () => {
    tokenStore.setRefreshCsrfToken("logout-csrf-value");
